...
export default class Analyzer {
  ...
  emitAudio = () => {
    for (let i = 0; i < this.onCallbacks.audio.length; i++)
      this.onCallbacks.audio[i](this);
  };
  ...
  update = (dt) => {
    ...
    for (let i = 0; i < BINS; i++)
      this.volumeByFrequency[i] = this.freqBytes[i] / 255;
    let s = 0;
    for (let i = 0; i < BINS; i++) {
    ...
    this.volume = Math.min(
      (rms / Math.max(this.agc, 0.05)) * params.volumeMult,
      1,
    );
    ...
    this.volumeSmooth +=
      (this.volume - this.volumeSmooth) * (1 - Math.pow(1 - rate, dt * 120));

    // average energy over [startBin, endBin)- the per-track band (≈86 Hz/bin, see TRACK_PARAMS)
    let low = 0;
    for (let i = params.startBin; i < params.endBin; i++)
      low += this.freqBytes[i];
    ...
    this.kickTimer += dt;
    if (
      this.kickTimer >= params.beatHold &&
      low > this.kickThreshold &&
      low > params.beatMin
    ) {
      this.kick = 1;
